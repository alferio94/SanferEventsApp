...
const GroupSelector = ({ route, navigation }) => {
  ...
  const fetchUserGroups = async () => {
    try {
      ...
      if (assignments && assignments.groups) {
        setUserGroups(assignments.groups);
      } else {
  ...
  const handleGroupPress = (group) => {
    ...
    const destination = mode === 'transport' ? 'transport' : 'agenda';
      ...
      >
        <View style={styles.groupCardContent}>
          <View style={styles.groupInfo}>
            <View style={[styles.groupColorIndicator, { backgroundColor: group.color || GlobalStyles.primary500 }]} />
